...
export function relevantArguments(args: BrowserParams): string[] {
	...
	if (args.timeoutMs !== undefined && args.timeoutMs !== 120_000) values.push(`timeout ${args.timeoutMs} ms`);
	return values;
}
...
function collapsedSummary(details: BrowserDetails | undefined, args: BrowserParams, theme: Theme): string {
	...
	if (details?.screenshot) {
		...
		metadata.push(preview && preview !== full ? `screenshot ${full} → preview ${preview}` : `screenshot ${full}`);
	}
...
export function renderBrowserResult(
	...
	if (details?.screenshot) {
		const shot = details.screenshot;
		let dimensions = `${shot.fullWidth}×${shot.fullHeight} · ${formatSize(shot.bytes)}`;
		if (shot.previewWidth && shot.previewHeight) dimensions += ` · preview ${shot.previewWidth}×${shot.previewHeight}`;
